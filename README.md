...
载荷—航程采用题面给出的 3/2 次关系。单点往返任务去程载荷为该架次全部货箱质量，返程载荷为 0。水平能耗按可用电量与等效航程折算，爬升能耗按重力势能除以爬升效率计算。组批目标依次为最少架次数、最小总能耗和最小累计作业时间。

运行后将生成最大安全载荷、航段几何、逐箱组批、组批汇总和两张 PNG 图。任何正式提交前都应核对赛题是否发布了更具体的水平能耗或爬升能耗补充公式。
