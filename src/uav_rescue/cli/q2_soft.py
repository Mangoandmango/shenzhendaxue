...
def _write_schedule_tables(
    ...
    write_csv(table_dir / "Pareto前沿.csv", [
        "Pareto点", "普通物资加权迟到", "完工时间（s）", "总能耗（kWh）",
        "架次数", "归一化及时性", "归一化完工时间", "归一化能耗",
        "归一化架次", "理想点距离", "代表方案",
    ], [
        [
...
def run(
    ...
    q2 = load_toml("configs/q2_soft.toml")
    paths = base["paths"]
    ...
    metadata = {
        ...
        "selection_rule": "医疗和首批保障时限为硬约束；普通物资期望送达时间为软约束；四目标Pareto前沿上min-max归一化等权理想点距离最小",
